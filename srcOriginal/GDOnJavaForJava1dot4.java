...
public class GDOnJavaForJava1dot4 extends MIDlet implements CommandListener { 
	...
	*/
	...
	class SoundForm extends Form {
		...
		public SoundForm() {
			...
			setCommandListener(GDOnJavaForJava1dot4.this);
	...
	class PlayScreen extends GameCanvas implements Runnable, CommandListener {
	 ...
	 */
		...
		TextBox mode;
		TextBox speed;
		Command Back = new Command("Return", Command.BACK, 2);
		Command Save = new Command("Save", Command.OK, 1);
		Command Okay = new Command("OK", Command.OK, 1);
		
		Command GravityAndSize = new Command("Gravity and Size", Command.OK, 1);
		...
		public void flagChooser() {
			...
			gravityAndSize = new TextBox("Value:", "", 2, 2);
			mode = new TextBox("Value:", "", 3, 2);
			speed = new TextBox("Value:", "", 3, 2);
			...
			saved = new Alert("Saved successfully", "The value has been saved", null, AlertType.INFO);
			error = new Alert("Failed to save", "The value you have typed is invalid", null, AlertType.INFO);
			
			display.setCurrent(FlagChooser);
		...
		public void commandAction(Command c, Displayable d) {
			...
			if (c == GravityAndSize) {
				...
				display.setCurrent(gravityAndSize);
				
			} else if (c == Mode) {
				
				display.setCurrent(mode);
				
			} else if (c == Speed) {
				
				display.setCurrent(speed);
				...
				try {
					...
					if (d == arr[0]) {
						...
						display.setCurrent(error, arr[0]);
						
					} else if (d == arr[1]) {
						
						display.setCurrent(error, arr[1]);
						
					} else if (d == arr[2]) {
						
						display.setCurrent(error, arr[2]);
